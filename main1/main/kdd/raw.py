...
import sys,csv,pickle,time,collections,copy
...
class Duplicates:
    ...
    def marry(self,a,b,dryrun=False):
        if b in self.duplicates[a]: return self.duplicates[a],set()
        new_aids,added_links = copy.copy(self.duplicates[a]),set()
        ...
        for duplicate_of_a in self.duplicates[a]:
            ...
            for duplicate_of_b in self.duplicates[b]:
                if not duplicate_of_b in self.duplicates[duplicate_of_a]:
                    added_links.add((duplicate_of_a,duplicate_of_b))
        ...
        for duplicate_of_b in self.duplicates[b]:
            ...
            for duplicate_of_a in self.duplicates[a]:
                if not duplicate_of_a in self.duplicates[duplicate_of_b]:
                    added_links.add((duplicate_of_b,duplicate_of_a))
                    if not dryrun: self.duplicates[duplicate_of_b].add(duplicate_of_a)
        for duplicate_of_b in self.duplicates[b]:
            if not duplicate_of_b in self.duplicates[a]:
                new_aids.add(duplicate_of_b)
                added_links.add((a,duplicate_of_b))
                if not dryrun: 
                    self.duplicates[a].add(duplicate_of_b)
        for duplicate_of_a in self.duplicates[a]:
            if not duplicate_of_a in self.duplicates[b]:
                added_links.add((b,duplicate_of_a))
                if not dryrun: self.duplicates[b].add(duplicate_of_a)
        return new_aids,added_links 
    def divorce(self,a):
